...
class Weather(BaseCog):
    ...
    @commands.slash_command(name="weather_location_select", description="查找天氣")
    async def choose_location(
        self,
        ctx: discord.ApplicationContext,
        location: discord.Option(str, "選擇要查找天氣的地方", choices=get_location_json()),
    ):
        """處理 /weather_location_select 指令"""
        try:
            with open("location.json", "r", encoding="utf-8") as f:
                locations = json.load(f)
            ...
            if isinstance(weather_elements, dict):
                ...
                embed.description = weather_elements
